...
use Cmf\Foundation\ErrorHandling\LogReporter;
...
class Server
{
    ...
    /**
     * Try to boot PHPCmf, and retrieve the app's HTTP request handler.
     *
    ...
    private function cleanBootExceptionLog(Throwable $error)
    {
        ...
        if ($container->has('cmf.config') && resolve('cmf.config')->inDebugMode()) {
            ...
            echo <<<ERROR
            PHPCmf encountered a boot error ($type)<br />
            <b>$message</b><br />
...
ERROR;
            ...
            echo 'PHPCmf encountered a boot error. Details have been logged to the PHPCmf log file.';
            exit(1);
        }
    }
    ...
    {
        echo 'PHPCmf encountered a boot error. Details have been logged to the system PHP log file.<br />';
