...
class CookieFactory
{
    ...
    {
        // If necessary, we will use the site's base URL to determine smart defaults for cookie settings
        $url = $config->url();

        // Get the cookie settings from the config or use the default values
        $this->prefix = $config['cookie.name'] ?? 'cmf';
     ...
     * It will be pre-configured according to PHPCmf's base URL and protocol.
     *
     * @param  string  $name
     * @param  string  $value
     * @param  int     $maxAge
