...
class User implements ExtenderInterface
{
     ...
     * @param string $driver: ::class attribute of driver class, which must implement Cmf\User\DisplayName\DriverInterface
     * @return self
     */
     ...
     * - \Cmf\User\User $user: the user in question.
    ...
    {
        $container->extend('cmf.user.display_name.supported_drivers', function ($existingDrivers) {
            return array_merge($existingDrivers, $this->displayNameDrivers);
        });

        $container->extend('cmf.user.group_processors', function ($existingRelations) {
            return array_merge($existingRelations, $this->groupProcessors);
        });

        foreach ($this->preferences as $key => $preference) {
            CmfUser::registerPreference($key, $preference['transformer'], $preference['default']);
        }
    }
}
