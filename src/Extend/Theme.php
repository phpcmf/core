...
class Theme implements ExtenderInterface
{
     ...
     * For example, core's `site.less` file imports a `site/DiscussionListItem.less` file.
     * The contents of this file can be overriden with this method.
     *
     * @param string $file : Relative path of the file to override, for example: `site/Hero.less`
     * @param string $newFilePath : Absolute path of the new file.
     * @param string|null $extensionId : If overriding an extension file, specify its ID, for example: `cmf-tags`.
     * @return self
     */
     ...
     * For example `site.less`, `admin.less`, `mixins.less` and `variables.less` are file sources,
     ...
     * ```php
     * (new Extend\Theme)
     *     ->addCustomLessFunction('is-cmf', function (mixed $text) {
     *         return strtolower($text) === 'cmf'
     *     }),
     * ```
     *
    ...
    {
        $container->extend('cmf.frontend.custom_less_functions', function (array $customFunctions) {
            return array_merge($customFunctions, $this->customFunctions);
        });

        $container->extend('cmf.less.custom_variables', function (array $lessVariables) {
            return array_merge($this->lessVariables, $lessVariables);
        });

        $container->extend('cmf.assets.factory', function (callable $factory) {
