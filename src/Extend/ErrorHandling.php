...
class ErrorHandling implements ExtenderInterface
{
     ...
     * itself (if it implements {@see \Cmf\Foundation\KnownError}), or
     ...
     * the exception implement our {@see \Cmf\Foundation\KnownError}
     ...
     *
     * When Cmf's default error handling is not enough for you, and the other
     ...
     * returns a {@see \Cmf\Foundation\ErrorHandling\HandledError} instance.
     ...
     *
     * Reporters will be called whenever Cmf encounters an exception that it
     ...
     * {@see \Cmf\Foundation\ErrorHandling\Reporter} interface.
     *
    ...
    public function extend(Container $container, Extension $extension = null)
    {
        if (count($this->statuses)) {
            $container->extend('cmf.error.statuses', function ($statuses) {
                return array_merge($statuses, $this->statuses);
            });
        }

        if (count($this->types)) {
            $container->extend('cmf.error.classes', function ($types) {
                return array_merge($types, $this->types);
            });
        }

        if (count($this->handlers)) {
            $container->extend('cmf.error.handlers', function ($handlers) {
