...
use Cmf\Frontend\Frontend as ActualFrontend;
...
class Frontend implements ExtenderInterface
{
     ...
     * - \Cmf\Frontend\Document $document
    ...
    private function registerAssets(Container $container, string $moduleName): void
    {
        ...
        $abstract = 'cmf.assets.'.$this->frontend;
            ...
            if ($this->js) {
                $assets->js(function (SourceCollector $sources) use ($moduleName) {
                    ...
                    $sources->addString(function () use ($moduleName) {
                        return "cmf.extensions['$moduleName']=module.exports;";
                    });
                });
            }
        ...
        if (! $container->bound($abstract)) {
            $container->bind($abstract, function (Container $container) {
                return $container->make('cmf.assets.factory')($this->frontend);
            });
    ...
    {
        if ($this->titleDriver) {
            $container->extend('cmf.frontend.title_driver', function ($driver, Container $container) {
