...
class Console implements ExtenderInterface
{
     ...
     * 将命令添加到控制台。
     *
     * @param string $command: ::class attribute of command class, which must extend Cmf\Console\AbstractCommand.
     * @return self
     */
    ...
    {
        $container->extend('cmf.console.commands', function ($existingCommands) {
            return array_merge($existingCommands, $this->addCommands);
        });

        $container->extend('cmf.console.scheduled', function ($existingScheduled) use ($container) {
