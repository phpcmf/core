...
class InstalledApp implements AppInterface
{
    ...
    public function getRequestHandler()
    {
        if ($this->config->inMaintenanceMode()) {
            return $this->container->make('cmf.maintenance.handler');
        ...
        $pipe->pipe(
            new BasePathRouter([
                $this->subPath('api') => 'cmf.api.handler',
                $this->subPath('admin') => 'cmf.admin.handler',
                '/' => 'cmf.site.handler',
            ])
        );
    ...
    protected function getUpdaterHandler()
    {
        ...
        $pipe->pipe(new BasePath($this->basePath()));
        $pipe->pipe(
            new HttpMiddleware\ResolveRoute($this->container->make('cmf.update.routes'))
        );
    ...
    public function getConsoleCommands()
    {
        return array_map(function ($command) {
            ...
            return $command;
        }, $this->container->make('cmf.console.commands'));
    }
}
