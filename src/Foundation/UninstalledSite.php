...
use Cmf\Install\Installer;
use Cmf\Install\InstallServiceProvider;
use Cmf\Locale\LocaleServiceProvider;
use Cmf\Settings\SettingsRepositoryInterface;
use Cmf\Settings\UninstalledSettingsRepository;
...
class UninstalledSite implements SiteInterface
{
    ...
    protected function bootLaravel(): Container
    {
        ...
        $container->instance('cmf.config', new Config(['url' => $this->baseUrl]));
        $container->alias('cmf.config', Config::class);
        $container->instance('cmf.debug', true);
    ...
    {
        $logPath = $this->paths->storage.'/logs/cmf-installer.log';
        $handler = new StreamHandler($logPath, Logger::DEBUG);
        $handler->setFormatter(new LineFormatter(null, null, true, true));

        $container->instance('log', new Logger('Cmf Installer', [$handler]));
