...
use Cmf\Frontend\Driver\BasicTitleDriver;
use Cmf\Frontend\Driver\TitleDriverInterface;
...
class FrontendServiceProvider extends AbstractServiceProvider
{
    ...
    {
        $this->container->singleton('cmf.assets.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $paths = $container[Paths::class];

                $assets = new Assets(
                    $name,
                    $container->make('filesystem')->disk('cmf-assets'),
                    $paths->storage,
                    null,
                    $container->make('cmf.frontend.custom_less_functions')
                );
        ...
        $this->container->singleton('cmf.frontend.factory', function (Container $container) {
            return function (string $name) use ($container) {
                $frontend = $container->make(Frontend::class);

                $frontend->content(function (Document $document) use ($name) {
                    $document->layoutView = 'cmf::frontend.'.$name;
                });
                ...
                $frontend->content(function (Document $document) use ($container) {
                    $default_preloads = $container->make('cmf.frontend.default_preloads');
        ...
        $this->container->singleton(
            'cmf.frontend.default_preloads',
            function (Container $container) {
                $filesystem = $container->make('filesystem')->disk('cmf-assets');
        ...
        $this->container->singleton(
            'cmf.frontend.custom_less_functions',
            function (Container $container) {
        ...
        $this->container->alias(TitleDriverInterface::class, 'cmf.frontend.title_driver');

        $this->container->singleton('cmf.less.config', function (Container $container) {
            return [
    ...
    {
        $this->loadViewsFrom(__DIR__.'/../../views', 'cmf');
    ...
    private function addLessVariables(SourceCollector $sources)
    {
        $sources->addString(function () {
            $vars = $this->container->make('cmf.less.config');
            $extDefinedVars = $this->container->make('cmf.less.custom_variables');
