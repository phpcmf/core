...
use Cmf\Extension\Event\Enabling;
use Cmf\Extension\Event\Uninstalled;
use Cmf\Extension\Exception\CircularDependenciesException;
...
class ExtensionManager
{
    ...
    public function getExtensions()
    {
        if (is_null($this->extensions) && $this->filesystem->exists($this->paths->vendor.'/composer/installed.json')) {
            ...
            // We calculate and store a set of composer package names for all installed PHPCmf extensions,
            // so we know what is and isn't a PHPCmf extension in `calculateDependencies`.
            ...
            foreach ($installed as $package) {
                ...
                if (Arr::get($package, 'type') === 'cmf-extension') {
                    $composerJsonConfs[$packagePath] = $package;
                }

                if ($subextPaths = Arr::get($package, 'extra.cmf-subextensions', [])) {
                    foreach ($subextPaths as $subExtPath) {
                        ...
                        if (Arr::get($conf, 'type') === 'cmf-extension') {
    ...
    /**
     * Call on all enabled extensions to extend the PHPCmf application.
     *
     ...
     *      'valid' points to an ordered array of \Cmf\Extension\Extension
