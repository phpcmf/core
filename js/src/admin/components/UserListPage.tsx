...
/**
 * Admin page which displays a paginated list of all users on the site.
 */
export default class UserListPage extends AdminPage {
   ...
   * data provided by `AdminPayload.php`, or `cmf/statistics`
   * if installed.
   */
  ...
  columns(): ItemList<ColumnData> {
    ...
    columns.add(
      'username',
      {
        ...
        content: (user: User) => {
          const profileUrl = `${app.site.attribute('baseUrl')}/user/${user.slug()}`;
