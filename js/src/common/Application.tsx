...
import RequestError, { InternalCmfRequestOptions } from './utils/RequestError';
...
export type CmfScreens = 'phone' | 'tablet' | 'desktop' | 'desktop-hd';

export type CmfGenericRoute = RouteItem<any, any, any>;

export interface CmfRequestOptions<ResponseType> extends Omit<Mithril.RequestOptions<ResponseType>, 'extract'> {
  errorHandler?: (error: RequestError) => void;
  url: string;
  // TODO: [Cmf 2.0] Remove deprecated option
  /**
...
> = {
   ...
   *
   * @see https://phpcmf.cn/extend/frontend-pages.html#route-resolvers-advanced
   */
  path: `/${string}`;
} & (
  | {
...
export default class Application {
   ...
   */
  routes: Record<string, CmfGenericRoute> = {};
    ...
    this.initializers.toArray().forEach((initializer) => {
      try {
        ...
        const extension = initializer.itemName.includes('/')
          ? initializer.itemName.replace(/(\/cmf-ext-)|(\/cmf-)/g, '-')
  ...
  screen(): CmfScreens {
    ...
    return styles.getPropertyValue('--cmf-screen') as ReturnType<Application['screen']>;
  }
  ...
  protected transformRequestOptions<ResponseType>(cmfOptions: CmfRequestOptions<ResponseType>): InternalCmfRequestOptions<ResponseType> {
    const { background, deserialize, extract, modifyText, ...tmpOptions } = { ...cmfOptions };
    ...
    const options: InternalCmfRequestOptions<ResponseType> = {
   ...
   */
  request<ResponseType>(originalOptions: CmfRequestOptions<ResponseType>): Promise<ResponseType> {
   ...
   */
  protected requestErrorCatch<ResponseType>(error: RequestError, customErrorHandler: CmfRequestOptions<ResponseType>['errorHandler']) {
   ...
   * While not certain, a failing cross-origin request likely indicates a missing redirect to Cmf canonical URL.
