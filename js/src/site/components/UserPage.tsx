                ...
                <div className="sideNavContainer">
                  <AffixedSidebar>
                    <nav className="sideNav UserPage-nav">
                      <ul className="UserPageNavItem">{listItems(this.sidebarItems().toArray())}</ul>
                    </nav>
                  </AffixedSidebar>
  ...
  navItems() {
    ...
    items.add(
      ...
      <LinkButton href={app.route('user.posts', { username: user.slug() })} icon="far fa-comment">
        {app.translator.trans('core.site.user.posts_link')} <span className="Button-badge">{user.commentCount()}</span>
      </LinkButton>,
      100
    );
