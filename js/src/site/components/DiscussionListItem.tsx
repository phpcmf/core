...
export default class DiscussionListItem<CustomAttrs extends IDiscussionListItemAttrs = IDiscussionListItemAttrs> extends Component<CustomAttrs> {
  ...
  authorAvatarView(): Mithril.Children {
    ...
    return (
      <Tooltip
        text={app.translator.trans('core.site.discussion_list.started_text', { user, ago: humanTime(discussion.createdAt()) })}
        position="right"
      >
  ...
  replyCountItem() {
      ...
      return (
        <button className="Button--ua-reset DiscussionListItem-count" onclick={this.markAsRead.bind(this)}>
          ...
          <span className="visually-hidden">
            {app.translator.trans('core.site.discussion_list.unread_replies_a11y_label', { count: discussion.replyCount() })}
          </span>
        </button>
      );
    }
    ...
    return (
      <span className="DiscussionListItem-count">
        ...
        <span className="visually-hidden">
          {app.translator.trans('core.site.discussion_list.total_replies_a11y_label', { count: discussion.replyCount() })}
        </span>
      </span>
    );
  }
