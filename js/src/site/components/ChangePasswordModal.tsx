...
export default class ChangePasswordModal<CustomAttrs extends IInternalModalAttrs = IInternalModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('core.site.change_password.title');
  }
  ...
  fields() {
    ...
    fields.add('help', <p className="helpText">{app.translator.trans('core.site.change_password.text')}</p>);

    fields.add(
      'submit',
      <div className="Form-group">
        <Button className="Button Button--primary Button--block" type="submit" loading={this.loading}>
          {app.translator.trans('core.site.change_password.send_button')}
        </Button>
      </div>
    );
