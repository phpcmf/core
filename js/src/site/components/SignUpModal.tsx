...
export default class SignUpModal<CustomAttrs extends ISignupModalAttrs = ISignupModalAttrs> extends Modal<CustomAttrs> {
  ...
  title() {
    return app.translator.trans('core.site.sign_up.title');
  }
  ...
  fields() {
    ...
    const usernameLabel = extractText(app.translator.trans('core.site.sign_up.username_placeholder'));
    const emailLabel = extractText(app.translator.trans('core.site.sign_up.email_placeholder'));
    const passwordLabel = extractText(app.translator.trans('core.site.sign_up.password_placeholder'));
    ...
    items.add(
      ...
      <div className="Form-group">
        <Button className="Button Button--primary Button--block" type="submit" loading={this.loading}>
          {app.translator.trans('core.site.sign_up.submit_button')}
        </Button>
      </div>,
      -10
    );
  ...
  footer() {
    return [
      <p className="SignUpModal-logIn">{app.translator.trans('core.site.sign_up.log_in_text', { a: <a onclick={this.logIn.bind(this)} /> })}</p>,
    ];
  }
  ...
  onsubmit(e: SubmitEvent) {
    ...
    app
      .request({
        url: app.site.attribute('baseUrl') + '/register',
        method: 'POST',
        body,
